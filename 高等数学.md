...
如果一个微分方程通过整理后可得到y和x分离，得到：
$$
y'=\frac{dy}{dx} \quad g(y)dy=f(x)dx
$$
通过将等式两边同时积分，就可解出原方程
$$
...
不管起点，两个长度(大小)相等，方向相同的量，称为两个相等的向量。
...
<img src="./assets/1284.jpg" style="zoom: 25%;" />
...
$$
\begin{flalign*}
&模:r=\sqrt{x^2+y^2+z^2}&\\
&两点距离:d=\sqrt{(x_1-x_2)^2+(y_1-y_2)^2+(z_1-z_2)^2}&
\end{flalign*}
$$
...
方向余弦cos(α)，α是向量v与坐标轴之间的夹角，cos(α)表示v在x轴上的投影与v的长度之比。
...
**如何求偏导：**求对x的偏导：视y为常数，对x求导



**偏导的几何含义：**y=y0这个曲线的切线的斜率

二元函数，对x的偏导存在，在y0的曲线上连续
...
$$
\begin{flalign*}
&z=f(x,y)在(x,y)的某个邻域内有连续的偏导数，f_x'(x,y),f_y'(x,y),&\\&则在(x,y)处可微，&\\
...
![image-20230910142026450](./assets/image-20230910142026450.png)
...
对x的偏导：垂直于y轴的切面而形成的曲线，曲线在这一点切线的斜率
...
$$
\begin{flalign*}
&以P_0(x_0,y_0)为起点的射线：&\\&x=x_0+tcos\alpha&\\
&y=y_0+tcos\beta&\\
&定理：f(x,y)在f(x_0,y_0)可微，方向导数存在且&\\
...
$$
(f_x'(x_0,y_0),f_y'(x_0,y_0))=gradf(x_0,y_0)=\nabla f(x_0,y_0)
$$
...
考虑一座高度在(x,y)点是H(x,y)的山。H这一点的梯度是在该点坡度(斜度)最陡的方向。梯度的大小告诉我们坡度到底多陡。
...
**定理1(必要)：**如果z=f(x,y)有偏导，并且(x0,y0)有极值，那么对x的偏导是0，对y的偏导也是0
$$
f_x'(x_0,y_0) = 0 \quad f_y'(x_0,y_0) = 0
$$
**驻点：**偏导等于0的点(x和y的偏导同时成立)
...
$$
\begin{flalign*}
&定理2:A=f_{xx}''(x_0,y_0)\quad B=f_{xy}''(x_0,y_0) \quad C=f_{yy}''(x_0,y_0)& \\
&(1)AC-B^2>0 \quad 极值 \quad A<0是极大值 \quad A>0是极小值& \\
&(1)AC-B^<0 \quad 没有极值& \\
&(1)AC-B^2=0 \quad 无法判断& \\
\end{flalign*}
$$
