...
```sql
INSERT INTO student(id,name,age) VALUES(4,'周杰伦',40);
```
...
```sql
SELECT id,name FROM student; 	#查看id和name两列的数据
```
...
```sql
SELECT gender,AVG(age) FROM student GROUP BY gender;  #按照性别分组，查询平均年龄
```

注意：使用聚合函数时，**group by**中写了哪个字段，这个字段才能出现在SELECT后面
...
```sql
SELECT * FROM student WHERE age>20 ORDER BY age DESC; 	#按照降序
```
...
```sql
SELECT * FROM student limit 10,5;	#前10条跳过，限制显示5条
```
...
# 获取游标对象
cursor = conn.cursor()
conn.select_db("practice")      # 先选择数据库
# 使用游标对象，执行sql语句
...
一个委托构造函数使用它所**属类的其他构造函数**执行它自己的初始化过程。
...
因为**静态数据成员**不属于类的任何一个对象，所以它们并不是在创建类的对象时被定义的。这意味着它们不是由类的构造函数初始化的。
